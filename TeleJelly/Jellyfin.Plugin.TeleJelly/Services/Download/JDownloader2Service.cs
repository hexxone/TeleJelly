...
public class JDownloader2Service : IHostedDownloadService
{
    ...
    public async Task<string> AddDownloadAsync(string linkOrFile, CancellationToken ct)
    {
        ...
        var result = await _client.LinkGrabberV2.AddLinks(new AddLinksQuery { Links = linkOrFile, AutoStart = true });
        ...
        // TODO test this logic and see if it even makes sense ? because link grabbing might take a while...
        var collected = await _client.LinkGrabberV2.QueryPackages(new CrawledPackageQuery([result.Id])) ?? [];

        var newPackage = collected.OrderByDescending(p => p.Uuid).FirstOrDefault();
