...
namespace Jellyfin.Plugin.TeleJelly.Services.Download.Search
{
    public class QualityRuleEngine
    {
        ...
        public double ScoreResult(SearchResult result, QualityProfile profile)
        {
            ...
            // 2. Codec & HDR
            if (profile.PreferredCodecs.Contains(result.Codec)) score += 500;
            if (!string.IsNullOrEmpty(result.HDR) && profile.PreferredHDR.Contains(result.HDR)) score += 300;
            ...
            {
                if (result.FileSizeBytes > maxSize) return 0; // Disqualify
            }
            ...
            if (profile.PreferredSources.Contains(result.Source)) score += 200;
