...
internal enum SearchDiscoveryMode
{
    WordPressRest,
    WordPressHtml
}

internal enum PostFetchMode
{
    WordPressJson,
    Html,
    HdEncodeProtectedHtml
}
...
internal class ConfigurableStructuredSearchProvider : ISearchProvider
{
    ...
    private async Task<IReadOnlyList<PageCandidate>> DiscoverWordPressRestCandidatesAsync(string term, CancellationToken ct)
    {
        ...
        var responseText = await _httpClient.GetStringAsync(restUrl, ct);
    ...
    private async Task<PostDocument?> LoadWordPressJsonDocumentAsync(PageCandidate candidate, CancellationToken ct)
    {
        ...
        var responseText = await _httpClient.GetStringAsync(candidate.DetailUrl, ct);
        if (!LooksLikeJson(responseText))
        {
            return await LoadHtmlDocumentAsync(candidate.PageUrl, candidate.Title, ct);
        }
    ...
    {
        var lockedHtml = await _httpClient.GetStringAsync(pageUrl, ct);
        var unlockPayload = ExtractHdEncodeUnlockPayload(lockedHtml);
        var finalHtml = lockedHtml;

        if (unlockPayload.Count > 0)
        {
            using var content = new FormUrlEncodedContent(unlockPayload);
            using var response = await _httpClient.PostAsync(pageUrl, content, ct);
            finalHtml = await response.Content.ReadAsStringAsync(ct);
        }
    ...
    private IEnumerable<SearchResult> BuildSearchResults(PostDocument document)
    {
        ...
        var fileSizeBytes = ParseLargestSizeBytes(decodedText);
    ...
    private static string? ExtractPassword(string html, string decodedText)
    {
    ...
    private static string[] ExtractLanguages(string text, bool audio)
    {
        var source = text;
        if (audio)
        {
    ...
    private static string? ParseCodec(string text)
    {
    ...
    private static string? ParseHdr(string text)
    {
    ...
    private static long ParseLargestSizeBytes(string text)
    {
        long best = 0;
        foreach (Match match in SizeRegex.Matches(text))
        {
    ...
    private Dictionary<string, string> ExtractHdEncodeUnlockPayload(string html)
    {
    ...
    private static DateTime? ExtractPostedDate(string html)
    {
